...
class ConfigGenerator(Loggable):
    ...
    def __init__(
        ...
        Args:
            ...
            odin_dir (Path): Odin config output directory
            epics_dir (Path): EPICS config output directory
        ...
        self.mark = mark
        self.odin_dir = odin_dir
        self.epics_dir = epics_dir
        self.test = test

        if self.test:
            # If testing then update the EDL directory to just use the EPICS dir
            self.adodin_edl_dir = self.epics_dir

    def clean(self) -> None:
        """Clean the target configuration directory"""
        self.logger.info(f"Cleaning directory: {self.odin_dir}")
        for file in glob(f"{self.odin_dir}/*"):
            os.remove(file)

    def generate(self) -> None:
        """Generate the configuration for Odin and related software"""
        self.logger.info(f"Generating files to: {self.odin_dir}")
        ...
        for func_cards_and_channels in funcs_with_cards_and_channels:
            func_cards_and_channels(
                self.num_cards, self.num_chans, self.template_dir, self.odin_dir
            )

        for func_with_cards_only in funcs_with_cards_only:
            func_with_cards_only(self.num_cards, self.template_dir, self.odin_dir)
    ...
    def __copy_common_files(self) -> None:
        ...
        for file in glob(f"{self.common_dir}/*.sh"):
            self.logger.debug(f"Copying {file} to {self.odin_dir}")
            shutil.copy2(file, self.odin_dir)

...
class ConfigGenerator:
    def __init__(
        self, num_cards, num_chans, mark, template_dir, common_dir, target_dir, test
    ):
    ...
    def run(self):
        ...
        xspress_expanded_substitutions(
            self.num_cards, self.num_chans, self.template_dir, self.test
        )
        create_gui(self.num_cards, self.template_dir, self.test)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Provide arguments for the number of channels & cards"
    )
    parser.add_argument(
        "-c",
        "--channels",
        type=int,
        default=8,
        help="The number of channels the system is",
    )
    parser.add_argument(
        "-m", "--mark", type=int, default=2, help="Mark of the system, Mk1 or Mk2"
    )
    parser.add_argument(
        "-d",
        "--dir",
        type=str,
        default="/odin/config",
        help="Directory where the file will be generated to",
    )
    parser.add_argument(
        "-t",
        "--test",
    ...
    if not args.test:
        if args.dir == "/odin/config" and not os.path.isdir("/odin/config"):
            ...
            print(
                "/odin/config/ directory doesn't exist,"
                " have you run the required previous steps?"
            )
            return
        target_dir = args.dir
    else:
        print("\n\nTEST MODE\n\n")
        target_dir = f"{module_dir}/config"
        edl_dir = f"{module_dir}/support/ADOdin/edl"
        subst_dir = f"{module_dir}/support/ADOdin/expanded_substitustions/"
        dirs = [target_dir, edl_dir, subst_dir]

        for dir in dirs:
            if not os.path.exists(dir):
                os.makedirs(dir)
        print(f"Testing so building files in {target_dir} ")
